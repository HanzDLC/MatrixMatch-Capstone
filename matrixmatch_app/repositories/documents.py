...
def add_document(title: str, academic_program: str, abstract: str, cursor=None) -> Optional[int]:
    if cursor is None:
        with db_cursor(commit=True) as cursor:
            return add_document(title, academic_program, abstract, cursor=cursor)
...
def update_document(document_id: int, title: str, academic_program: str, abstract: str, cursor=None) -> bool:
    if cursor is None:
        with db_cursor(commit=True) as cursor:
            return update_document(document_id, title, academic_program, abstract, cursor=cursor)
...
def delete_document(document_id: int, cursor=None) -> bool:
    if cursor is None:
        with db_cursor(commit=True) as cursor:
            return delete_document(document_id, cursor=cursor)
